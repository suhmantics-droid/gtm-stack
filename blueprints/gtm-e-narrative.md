...
## The core narrative
- **Systems, not headcount.** The old answer to "we need more pipeline" was "hire more SDRs."
  The GTM-engineering answer is "build a system." A fully-loaded SDR runs ~$180K/yr for a handful
...
1. **The loop:** signal → enrich → score → route → act → measure → (kill | template). Every play
   is a variation of this. (`../gtm-engineering/PLAYBOOK.md` §2.)
2. **The waterfall:** query data providers cheapest→dearest, stop at first verified hit, pay only
   for hits. Tuned waterfalls take valid-data rates from ~55% to 90%+. (Blueprint 1.)
3. **Maturity rungs:** Foundation → Modeling → Activation. Don't sell activation to a business whose
...
4. Reframe vs competitors by **category**, not feature-by-feature (e.g. "a signal system" vs "another tool").
